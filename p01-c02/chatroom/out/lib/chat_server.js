...
function joinRoom(socket, room) {
  // Join the room.
  socket.join(room); // Associate id to room.
  ...
  const usersInRoom = io.sockets.clients(room); // If there is more than one user.
  ...
  socket.on('nameAttempt', name => {
    ...
    if (name.indexOf('Guest') === 0) {
      ...
      if (namesUsed.indexOf(name) === -1) {
        ...
        const previousNameIndex = namesUsed.indexOf(previousName); // Delete previous name from state.

        delete namesUsed[previousNameIndex]; // Add new name to state.

        nickNames[socket.id] = name;
        namesUsed.push(name); // Notify the results.
